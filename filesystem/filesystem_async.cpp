...
static CThreadFastMutex g_AsyncCallbackMutex;
...
void CBaseFileSystem::DoAsyncCallback( const FileAsyncRequest_t &request, void *pData, int nBytesRead, FSAsyncStatus_t result )
{
	...
	{
		AUTO_LOCK_FM( g_AsyncCallbackMutex );
...
//-----------------------------------------------------------------------------
// Synchronous implementation of the CS:GO async directory scan API.
//-----------------------------------------------------------------------------
static void ScanDirectoryRecursive( CBaseFileSystem *pFileSystem, const char *pSearchSpec, bool bRecurse, void *pContext, FSAsyncScanAddFunc_t pfnAdd )
{
	char szDir[MAX_PATH];
	V_ExtractFilePath( pSearchSpec, szDir, sizeof( szDir ) );
	char szWildcard[MAX_PATH];
	V_strncpy( szWildcard, V_UnqualifiedFileName( pSearchSpec ), sizeof( szWildcard ) );

	FileFindHandle_t hFind = -1;
	for ( const char *pszName = pFileSystem->FindFirst( pSearchSpec, &hFind ); pszName; pszName = pFileSystem->FindNext( hFind ) )
	{
		if ( pszName[0] == '.' )
			continue;
		if ( pFileSystem->FindIsDirectory( hFind ) )
			continue;
		char szFoundPath[MAX_PATH];
		V_strncpy( szFoundPath, szDir, sizeof( szFoundPath ) );
		char szFoundFile[MAX_PATH];
		V_strncpy( szFoundFile, pszName, sizeof( szFoundFile ) );
		pfnAdd( pContext, szFoundPath, szFoundFile );
	}
	pFileSystem->FindClose( hFind );

	if ( bRecurse )
	{
		char szSubSearch[MAX_PATH];
		V_snprintf( szSubSearch, sizeof( szSubSearch ), "%s*", szDir );
		FileFindHandle_t hDirs = -1;
		for ( const char *pszName = pFileSystem->FindFirst( szSubSearch, &hDirs ); pszName; pszName = pFileSystem->FindNext( hDirs ) )
		{
			if ( pszName[0] == '.' )
				continue;
			if ( !pFileSystem->FindIsDirectory( hDirs ) )
				continue;
			char szSub[MAX_PATH];
			V_snprintf( szSub, sizeof( szSub ), "%s%s%c%s", szDir, pszName, CORRECT_PATH_SEPARATOR, szWildcard );
			ScanDirectoryRecursive( pFileSystem, szSub, true, pContext, pfnAdd );
		}
		pFileSystem->FindClose( hDirs );
	}
}
...
FSAsyncStatus_t CBaseFileSystem::AsyncDirectoryScan( const char* pSearchSpec, bool recurseFolders, void* pContext, FSAsyncScanAddFunc_t pfnAdd, FSAsyncScanCompleteFunc_t pfnDone, FSAsyncControl_t *pControl )
{
	...
	if ( pfnAdd )
		ScanDirectoryRecursive( this, pSearchSpec, recurseFolders, pContext, pfnAdd );

	if ( pfnDone )
		pfnDone( pContext, FSASYNC_OK );
