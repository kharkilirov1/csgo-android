...
static void DrawFlashlightPass( CBaseShader *pShader, IMaterialVar** params, IShaderDynamicAPI *pShaderAPI, IShaderShadow* pShaderShadow, int nPass, WorldVertexTransition_DX6_Vars_t &info )
{
	...
	DYNAMIC_STATE
	{
		...
		pShader->BindTexture( SHADER_SAMPLER0, TEXTURE_BINDFLAGS_NONE, FLASHLIGHTTEXTURE, FLASHLIGHTTEXTUREFRAME );
		if( nPass == 0 )
		{
			pShader->BindTexture( SHADER_SAMPLER1, TEXTURE_BINDFLAGS_NONE, info.m_nBaseTexture2Var, -1 );
		}
		else
		{
			pShader->BindTexture( SHADER_SAMPLER1, TEXTURE_BINDFLAGS_NONE, info.m_nBaseTextureVar, info.m_nBaseTextureFrameVar );
		}
	}
	pShader->Draw();
}
...
void DrawWorldVertexTransition_DX6( CBaseShader *pShader, IMaterialVar** params, IShaderDynamicAPI *pShaderAPI, IShaderShadow* pShaderShadow, WorldVertexTransition_DX6_Vars_t &info )
{
	...
	DYNAMIC_STATE
	{
		...
		pShader->BindTexture( SHADER_SAMPLER1, TEXTURE_BINDFLAGS_NONE, info.m_nBaseTextureVar );
		pShaderAPI->BindStandardTexture( SHADER_SAMPLER0, TEXTURE_BINDFLAGS_NONE, TEXTURE_LIGHTMAP );
	}
	pShader->Draw();
	...
	DYNAMIC_STATE
	{
		pShader->BindTexture( SHADER_SAMPLER1, TEXTURE_BINDFLAGS_NONE, info.m_nBaseTexture2Var );
		pShaderAPI->BindStandardTexture( SHADER_SAMPLER0, TEXTURE_BINDFLAGS_NONE, TEXTURE_LIGHTMAP );
	}
	pShader->Draw();
}
