...
BEGIN_VS_SHADER( Water_DX81, 
		...
		IShaderDynamicAPI* pShaderAPI, bool bReflection, bool bRefraction ) 
	{
		...
		DYNAMIC_STATE
		{
			...
			pShaderAPI->BindStandardTexture( SHADER_SAMPLER1, TEXTURE_BINDFLAGS_NONE, TEXTURE_NORMALIZATION_CUBEMAP );
			if( bRefraction )
			{
				BindTexture( SHADER_SAMPLER2, TEXTURE_BINDFLAGS_NONE, REFRACTTEXTURE, -1 );
			}
			BindTexture( SHADER_SAMPLER3, TEXTURE_BINDFLAGS_NONE, NORMALMAP, BUMPFRAME );
			if( bReflection )
			{
				BindTexture( SHADER_SAMPLER4, TEXTURE_BINDFLAGS_NONE, REFLECTTEXTURE, -1 );
			}
		                        ...
		                        IShaderDynamicAPI* pShaderAPI, DrawCheapType_t type )
	{
		...
		{
			pShaderAPI->SetDefaultState();
			BindTexture( SHADER_SAMPLER0, TEXTURE_BINDFLAGS_NONE, NORMALMAP, BUMPFRAME );
			BindTexture( SHADER_SAMPLER3, TEXTURE_BINDFLAGS_NONE, ENVMAP, ENVMAPFRAME );
			pShaderAPI->BindStandardTexture( SHADER_SAMPLER4, TEXTURE_BINDFLAGS_NONE, TEXTURE_NORMALIZATION_CUBEMAP );
