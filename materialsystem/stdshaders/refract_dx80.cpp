...
BEGIN_VS_SHADER( Refract_DX80, 
	...
	SHADER_DRAW
	{
		...
		DYNAMIC_STATE
		{
			...
			{
				BindTexture( SHADER_SAMPLER0, TEXTURE_BINDFLAGS_NONE, DUDVMAP, BUMPFRAME );
			}
			else
			{
				BindTexture( SHADER_SAMPLER0, TEXTURE_BINDFLAGS_NONE, DUDVMAP, DUDVFRAME );
			}
			...
			{
				pShaderAPI->BindStandardTexture( SHADER_SAMPLER1, TEXTURE_BINDFLAGS_NONE, TEXTURE_FRAME_BUFFER_FULL_TEXTURE_0 );
			}

			if( bRefractTintTexture )
			{
				BindTexture( SHADER_SAMPLER2, TEXTURE_BINDFLAGS_NONE, REFRACTTINTTEXTURE, REFRACTTINTTEXTUREFRAME );
			}
