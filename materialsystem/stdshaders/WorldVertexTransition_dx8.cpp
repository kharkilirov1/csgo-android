	...
	SHADER_DRAW
	{
		...
		else if ( params[SEAMLESS_SCALE]->GetFloatValue() != 0.0f )
		{
			...
			DYNAMIC_STATE
			{
				...
				if( bLightingOnly )
				{
					...
					pShaderAPI->BindStandardTexture( SHADER_SAMPLER1, TEXTURE_BINDFLAGS_NONE, TEXTURE_GREY );
					pShaderAPI->BindStandardTexture( SHADER_SAMPLER2, TEXTURE_BINDFLAGS_NONE, TEXTURE_GREY );
				}
				else
				{
					BindTexture( SHADER_SAMPLER0, TEXTURE_BINDFLAGS_NONE, BASETEXTURE, FRAME );
					BindTexture( SHADER_SAMPLER1, TEXTURE_BINDFLAGS_NONE, BASETEXTURE, FRAME );
					BindTexture( SHADER_SAMPLER2, TEXTURE_BINDFLAGS_NONE, BASETEXTURE, FRAME );
				}
			...
			DYNAMIC_STATE
			{
				if( !bLightingOnly )
				{
					BindTexture( SHADER_SAMPLER0, TEXTURE_BINDFLAGS_NONE, BASETEXTURE2, FRAME2 );
					BindTexture( SHADER_SAMPLER1, TEXTURE_BINDFLAGS_NONE, BASETEXTURE2, FRAME2 );
					BindTexture( SHADER_SAMPLER2, TEXTURE_BINDFLAGS_NONE, BASETEXTURE2, FRAME2 );
				}
			}
			Draw();
		}
		else if( !params[BUMPMAP]->IsTexture() || !g_pConfig->UseBumpmapping() )
		{
			...
			DYNAMIC_STATE
			{
				...
				if ( bSupports14 && bBlendModulate )
				{
					BindTexture( SHADER_SAMPLER4, TEXTURE_BINDFLAGS_NONE, BLENDMODULATETEXTURE );
				}
		...
		else
		{
			...
			else
			{
				...
				DYNAMIC_STATE
				{
					...
					pShaderAPI->BindStandardTexture( SHADER_SAMPLER1, TEXTURE_BINDFLAGS_NONE, TEXTURE_LIGHTMAP );
