...
BEGIN_VS_SHADER( Core_DX90, 
		...
		IShaderDynamicAPI* pShaderAPI, int nPass, VertexCompressionType_t vertexCompression ) 
	{
		...
		DYNAMIC_STATE
		{
			...
			{
				pShaderAPI->BindStandardTexture( SHADER_SAMPLER2, TEXTURE_BINDFLAGS_NONE, TEXTURE_FRAME_BUFFER_FULL_TEXTURE_0 );
			}
			...
			if( bHasEnvmap )
			{
				BindTexture( SHADER_SAMPLER4, TEXTURE_BINDFLAGS_NONE, ENVMAP, ENVMAPFRAME );
			}

			if( bHasFlowmap )
			{
				BindTexture( SHADER_SAMPLER6, TEXTURE_BINDFLAGS_NONE, FLOWMAP, FLOWMAPFRAME );
			}

			if( bHasCoreColorTexture )
			{
				BindTexture( SHADER_SAMPLER7, TEXTURE_BINDFLAGS_NONE, CORECOLORTEXTURE, CORECOLORTEXTUREFRAME );
			}
