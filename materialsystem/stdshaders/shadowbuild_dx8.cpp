...
BEGIN_VS_SHADER_FLAGS( ShadowBuild_DX8, "Help for ShadowBuild", SHADER_NOT_EDITABLE )
	...
	SHADER_DRAW
	{
		...
		DYNAMIC_STATE
		{
			...
			if (pTexture)
			{
				BindTexture( SHADER_SAMPLER0, TEXTURE_BINDFLAGS_NONE, pTexture, ppTranslucentParams[FRAME]->GetIntValue() );
			...
			{
				pShaderAPI->BindStandardTexture( SHADER_SAMPLER0, TEXTURE_BINDFLAGS_NONE, TEXTURE_LIGHTMAP_FULLBRIGHT );
			}
