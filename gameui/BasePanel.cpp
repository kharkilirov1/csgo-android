...
public:
	...
	virtual void OnKeyCodePressed( KeyCode code )
	{
		...
		if ( nDir != 0 )
		{
			// controller nav removed: no SortedPanel helpers in CS:GO vgui
		}
    ...
    MESSAGE_FUNC_PTR( OnCursorEnteredMenuItem, "CursorEnteredMenuItem", menuItem);
...
//-----------------------------------------------------------------------------
void CGameMenu::OnCursorEnteredMenuItem(vgui::Panel* menuItem)
{
	MenuItem *item = static_cast<MenuItem *>(menuItem);
...
static int CC_GameMenuCompletionFunc( char const *partial, char commands[ COMMAND_COMPLETION_MAXITEMS ][ COMMAND_COMPLETION_ITEM_LENGTH ] )
{
	...
	struct CUtlStringCaselessLess
	{
		static bool LessFunc( const CUtlString &lhs, const CUtlString &rhs )
		{
			return ( Q_stricmp( lhs.Get(), rhs.Get() ) < 0 );
		}
	};
	CUtlRBTree< CUtlString > symbols( 0, 0, CUtlStringCaselessLess::LessFunc );
...
void CBasePanel::UpdateBackgroundState()
{
	...
	if (m_bHaveDarkenedTitleText != bNeedDarkenedTitleText || m_bForceTitleTextUpdate)
	{
		...
		// Msg( "animating title (%d => %d at time %.2f)\n", m_pGameMenuButtons[0]->GetAlpha(), (int)targetTitleAlpha, Plat_FloatTime());
...
void CBasePanel::ApplySchemeSettings(IScheme *pScheme)
{
	...
	if ( IsPC() )
	{
		...
		if ( m_iLoadingImageID == -1 )
		{
			...
			if ( false ) // no Steam Deck detection in this tree
...
void CBasePanel::OnGameUIActivated()
{
	...
	if ( !m_bEverActivated )
	{
		...
		if ( XBX_GetPrimaryUserId() != XBX_INVALID_USER_ID && XBX_GetStorageDeviceId( 0 ) != XBX_INVALID_STORAGE_ID && XBX_GetStorageDeviceId( 0 ) != XBX_STORAGE_DECLINED )
		{
			...
			if ( nRet != ERROR_SUCCESS )
			{
				...
				XBX_SetStorageDeviceId( 0, XBX_INVALID_STORAGE_ID );
...
void CBasePanel::RunMenuCommand(const char *command)
{
	...
	else if ( !Q_stricmp( command, "StorageDeviceDenied" ) )
	{
		...
		// Set us as declined
		XBX_SetStorageDeviceId( 0, XBX_STORAGE_DECLINED );
		m_iStorageID = XBX_INVALID_STORAGE_ID;
...
bool CBasePanel::ValidateStorageDevice( void )
{
	if ( m_bUserRefusedStorageDevice == false )
	{
		...
		if ( XBX_GetStorageDeviceId( 0 ) == XBX_INVALID_STORAGE_ID )
		{
			...
			{
				// Take this device
				XBX_SetStorageDeviceId( 0, nFoundDevice );
				OnDeviceAttached();
			}
			// Fall through
		}
		...
		if ( m_bWaitingForStorageDeviceHandle )
		{
			...
			if ( ret != ERROR_IO_INCOMPLETE )
			{
				...
				if ( m_iStorageID != XBX_INVALID_STORAGE_ID )
				{
					...
					if ( xboxsystem->DeviceCapacityAdequate( 0, m_iStorageID, COM_GetModDirectory() ) == false )
					{
					...
					else
					{
						...
						XBX_SetStorageDeviceId( 0, m_iStorageID );
						OnDeviceAttached();
					}
				}
				else
...
void CBasePanel::OnChangeStorageDevice( void )
{
	if ( m_bWaitingForStorageDeviceHandle == false )
	{
		...
		m_iStorageID = XBX_INVALID_STORAGE_ID;
		xboxsystem->ShowDeviceSelector( 0, true, &m_iStorageID, &m_hStorageDeviceChangeHandle );
	}
}
