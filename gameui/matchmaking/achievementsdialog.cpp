...
CAchievementsDialog::~CAchievementsDialog()
{
	if ( achievementmgr )
	{
		achievementmgr->SaveGlobalStateIfDirty();		// check for saving here to store achievements we want pinned to HUD
	}
...
void CAchievementsDialog::UpdateAchievementDialogInfo( void )
{
	...
	if ( achievementmgr )
	{
		...
		{
			IAchievement* pCurAchievement = achievementmgr->GetAchievementByIndex( i, 0 );
