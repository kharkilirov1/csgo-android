...
SpewRetval_t CmdLib_SpewOutputFunc( SpewType_t type, char const *pMsg )
{
	...
	EnterCriticalSection( &g_SpewCS );
	{
		...
		{
				// The legacy per-message spew color accessor was removed when
				// tier0 moved to the channel-based logging API.
...
void InstallSpewFunction()
{
	...
	// Legacy SpewOutputFunc registration is unavailable in the current tier0.
	// Output still goes through tier0's installed logging listeners.
	GetInitialColors();
}
