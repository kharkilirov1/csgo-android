...
static void AddOcclusionLump( )
{
	...
	memset( lump->fourCC, 0, sizeof( lump->fourCC ) );
...
public:
	...
	bool EnumerateLeavesInSphereWithFlagSet( const Vector& center, float radius, ISpatialLeafEnumerator* pEnum, intp context, int nFlagsCheck );
	int ListLeavesInBox( const Vector& mins, const Vector& maxs, unsigned short *pList, int listMax );
	int ListLeavesInSphereWithFlagSet( int *pLeafsInSphere, const Vector& center, float radius, int nLeafCount, const uint16 *pLeafs, int nLeafStride, int nFlagsCheck );
};
...
class CFlaggedLeafEnumerator : public ISpatialLeafEnumerator
{
public:
	CFlaggedLeafEnumerator( ISpatialLeafEnumerator *pTarget, intp context, int flags )
		: m_pTarget( pTarget ), m_Context( context ), m_Flags( flags ) {}
	bool EnumerateLeaf( int leaf, intp )
	{
		return ( dleafs[leaf].flags & m_Flags ) == 0 ||
			m_pTarget->EnumerateLeaf( leaf, m_Context );
	}
private:
	ISpatialLeafEnumerator *m_pTarget;
	intp m_Context;
	int m_Flags;
};

bool CToolBSPTree::EnumerateLeavesInSphereWithFlagSet( const Vector& center, float radius,
	ISpatialLeafEnumerator* pEnum, intp context, int nFlagsCheck )
{
	CFlaggedLeafEnumerator filter( pEnum, context, nFlagsCheck );
	return EnumerateLeavesInSphere( center, radius, &filter, 0 );
}

class CLeafListEnumerator : public ISpatialLeafEnumerator
{
public:
	CLeafListEnumerator( unsigned short *pList, int maxCount )
		: m_pList( pList ), m_MaxCount( maxCount ), m_Count( 0 ) {}
	bool EnumerateLeaf( int leaf, intp )
	{
		if ( m_Count >= m_MaxCount )
			return false;
		m_pList[m_Count++] = (unsigned short)leaf;
		return true;
	}
...
int CToolBSPTree::ListLeavesInBox( const Vector& mins, const Vector& maxs,
	unsigned short *pList, int listMax )
{
	CLeafListEnumerator list( pList, listMax );
	EnumerateLeavesInBox( mins, maxs, &list, 0 );
	return list.m_Count;
}

int CToolBSPTree::ListLeavesInSphereWithFlagSet( int *pLeafsInSphere,
	const Vector& center, float radius, int nLeafCount, const uint16 *pLeafs,
	int nLeafStride, int nFlagsCheck )
{
	int count = 0;
	const byte *pLeaf = reinterpret_cast<const byte *>( pLeafs );
	const float radiusSqr = radius * radius;
	for ( int i = 0; i < nLeafCount; ++i, pLeaf += nLeafStride )
	{
		const int leaf = *reinterpret_cast<const uint16 *>( pLeaf );
		if ( leaf < 0 || leaf >= numleafs || ( dleafs[leaf].flags & nFlagsCheck ) == 0 )
			continue;
		float distanceSqr = 0.0f;
		for ( int axis = 0; axis < 3; ++axis )
		{
			float delta = 0.0f;
			if ( center[axis] < dleafs[leaf].mins[axis] )
				delta = dleafs[leaf].mins[axis] - center[axis];
			else if ( center[axis] > dleafs[leaf].maxs[axis] )
				delta = center[axis] - dleafs[leaf].maxs[axis];
			distanceSqr += delta * delta;
		}
		if ( distanceSqr <= radiusSqr )
			pLeafsInSphere[count++] = i;
	}
	return count;
}
...
void ConvertPakFileContents( const char *pInFilename )
{
	...
	while ( 1 )
	{
		...
		{
			bOK = g_pVTFConvertFunc( relativeName, sourceBuf, targetBuf, g_pCompressFunc, 0 );
			if ( !bOK )
			{
...
bool CompressGameLump( BSPHeader_t *pInBSPHeader, BSPHeader_t *pOutBSPHeader, CUtlBuffer &outputBuffer, CompressFunc_t pCompressFunc )
{
	...
	memset( pOutBSPHeader->lumps[LUMP_GAME_LUMP].fourCC, 0, sizeof( pOutBSPHeader->lumps[LUMP_GAME_LUMP].fourCC ) );
...
bool RepackBSP( CUtlBuffer &inputBuffer, CUtlBuffer &outputBuffer, CompressFunc_t pCompressFunc, IZip::eCompressionType packfileCompression )
{
	...
	for ( int i = 0; i < HEADER_LUMPS; ++i )
	{
		...
		memset( sOutBSPHeader.lumps[lumpNum].fourCC, 0, sizeof( sOutBSPHeader.lumps[lumpNum].fourCC ) );
		...
		if ( pSortedLump->pLump->filelen ) // Otherwise its degenerate
		{
			...
			CUtlBuffer inputBuffer;
			byte *pSourceLump = ((byte *)pInBSPHeader) + pSortedLump->pLump->fileofs;
			if ( CLZMA::IsCompressed( pSourceLump ) )
			{
				unsigned int actualSize = CLZMA::GetActualSize( pSourceLump );
				inputBuffer.EnsureCapacity( actualSize );
				unsigned int outSize = CLZMA::Uncompress( pSourceLump, (unsigned char *)inputBuffer.Base() );
				inputBuffer.SeekPut( CUtlBuffer::SEEK_CURRENT, outSize );
			...
			else
			{
				...
				if ( bCompressed )
				{
					*(int *)sOutBSPHeader.lumps[lumpNum].fourCC = inputBuffer.TellPut();
