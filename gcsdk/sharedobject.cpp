...
bool CSharedObject::BAddToMessage( std::string *pBuffer ) const
{
	CUtlBuffer buffer;
	if ( !BAddToMessage( buffer ) )
		return false;
	pBuffer->assign( static_cast<const char *>( buffer.Base() ), buffer.TellPut() );
	return true;
}

bool CSharedObject::BAddDestroyToMessage( std::string *pBuffer ) const
{
	CUtlBuffer buffer;
	if ( !BAddDestroyToMessage( buffer ) )
		return false;
	pBuffer->assign( static_cast<const char *>( buffer.Base() ), buffer.TellPut() );
	return true;
}
