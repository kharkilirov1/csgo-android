...
class Android:
	...
	def cc(self):
		...
		compiler = self.gen_toolchain_path() + ('clang' if self.is_clang() else 'gcc')
		if sys.platform == 'win32' and self.is_clang():
			compiler = os.path.join(self.gen_gcc_toolchain_path(), 'bin', 'clang.exe')
			compiler += ' --target=%s%d' % (self.ndk_triplet(llvm_toolchain = True), self.api)
		return compiler

	def cxx(self):
		if self.is_host():
			return 'clang++ --target=%s%d' % (self.ndk_triplet(), self.api)
		compiler = self.gen_toolchain_path() + ('clang++' if self.is_clang() else 'g++')
		if sys.platform == 'win32' and self.is_clang():
			compiler = os.path.join(self.gen_gcc_toolchain_path(), 'bin', 'clang++.exe')
			compiler += ' --target=%s%d' % (self.ndk_triplet(llvm_toolchain = True), self.api)
		return compiler
	...
	def cflags(self, cxx = False):
		...
		cflags += ['-I%s'%i for i in self.system_stl()]+['-DANDROID', '-D_ANDROID', '-D__ANDROID__']
	...
	def ldflags(self):
		...
		if not self.is_clang():
			...
			if self.is_clang() or self.is_host():
				ldflags += ['-stdlib=libc++' if self.ndk_rev >= 18 else '-stdlib=libstdc++']
		if self.is_arm():
			if self.arch == 'armeabi-v7a':
...
def configure(conf):
	if conf.options.ANDROID_OPTS:
		...
		conf.env.CXXFLAGS += ['-Wno-c++11-narrowing']
