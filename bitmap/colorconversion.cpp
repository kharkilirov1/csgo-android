...
// Open replacement for the legacy nvtc binary encoder.  Keeping this entry
// point also preserves the ABI expected by older bitmap callers.
void S3TCencode( DDSURFACEDESC *pSrc, PALETTEENTRY *, DDSURFACEDESC *,
	void *pDestBuffer, unsigned int encodeType, float * )
{
	const int bytesPerPixel = pSrc->ddpfPixelFormat.dwRGBBitCount / 8;
	const bool explicitAlpha = ( encodeType & _S3TC_ENCODE_ALPHA_MASK ) == S3TC_ENCODE_ALPHA_EXPLICIT;
	const bool interpolatedAlpha = ( encodeType & _S3TC_ENCODE_ALPHA_MASK ) == S3TC_ENCODE_ALPHA_INTERPOLATED;
	const bool alphaCompare = ( encodeType & _S3TC_ENCODE_RGB_MASK ) == S3TC_ENCODE_RGB_ALPHA_COMPARE;
	const int outputBlockSize = ( explicitAlpha || interpolatedAlpha ) ? 16 : 8;
	byte *pOutput = static_cast<byte *>( pDestBuffer );

	for ( unsigned int blockY = 0; blockY < pSrc->dwHeight; blockY += 4 )
	{
		for ( unsigned int blockX = 0; blockX < pSrc->dwWidth; blockX += 4 )
		{
			byte rgba[64];
			for ( int y = 0; y < 4; ++y )
			{
				const unsigned int candidateY = blockY + y;
				const unsigned int sy = candidateY < pSrc->dwHeight ? candidateY : pSrc->dwHeight - 1;
				for ( int x = 0; x < 4; ++x )
				{
					const unsigned int candidateX = blockX + x;
					const unsigned int sx = candidateX < pSrc->dwWidth ? candidateX : pSrc->dwWidth - 1;
					const byte *pPixel = static_cast<const byte *>( pSrc->lpSurface ) + sy * pSrc->lPitch + sx * bytesPerPixel;
					byte *pRGBA = &rgba[( y * 4 + x ) * 4];
					if ( bytesPerPixel == 4 && pSrc->ddpfPixelFormat.dwRBitMask == 0x00ff0000 )
					{
						pRGBA[0] = pPixel[2]; pRGBA[1] = pPixel[1]; pRGBA[2] = pPixel[0]; pRGBA[3] = pPixel[3];
					}
					else
					{
						pRGBA[0] = pPixel[0]; pRGBA[1] = pPixel[1]; pRGBA[2] = pPixel[2];
						pRGBA[3] = bytesPerPixel == 4 ? pPixel[3] : 255;
					}
				}
			}
			...
			if ( explicitAlpha )
			{
				for ( int pair = 0; pair < 8; ++pair )
					pOutput[pair] = ( rgba[( pair * 2 ) * 4 + 3] >> 4 ) |
						( rgba[( pair * 2 + 1 ) * 4 + 3] & 0xf0 );
				stb__CompressColorBlock( pOutput + 8, rgba, STB_DXT_NORMAL );
			}
			else
			{
				stb_compress_dxt_block( pOutput, rgba, interpolatedAlpha ? 1 : 0, STB_DXT_NORMAL );
				if ( alphaCompare )
				{
					uint16 *pEndpoints = reinterpret_cast<uint16 *>( pOutput );
					if ( pEndpoints[0] > pEndpoints[1] )
					{
						const uint16 endpoint = pEndpoints[0];
						pEndpoints[0] = pEndpoints[1];
						pEndpoints[1] = endpoint;
						uint32 &indices = *reinterpret_cast<uint32 *>( pOutput + 4 );
						for ( int pixel = 0; pixel < 16; ++pixel )
						{
							const uint32 oldIndex = ( indices >> ( pixel * 2 ) ) & 3;
							const uint32 newIndex = oldIndex == 0 ? 1 : oldIndex == 1 ? 0 : 2;
							indices = ( indices & ~( 3u << ( pixel * 2 ) ) ) | ( newIndex << ( pixel * 2 ) );
						}
					}
					uint32 &indices = *reinterpret_cast<uint32 *>( pOutput + 4 );
					for ( int pixel = 0; pixel < 16; ++pixel )
						if ( rgba[pixel * 4 + 3] < 128 )
							indices = ( indices & ~( 3u << ( pixel * 2 ) ) ) | ( 3u << ( pixel * 2 ) );
				}
			}
			pOutput += outputBlockSize;
		}
	}
}
#endif
