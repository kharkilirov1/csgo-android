...
{
	f*=(1.0/overbright);
	int ival=std::min(255,(int)ceil(f*255.0));
	return ival;
}
					   ...
					   float newscale, float overbright)
{
	...
	// components.
	float maxc=std::max(std::max(orig.Pixel(x,y,0),orig.Pixel(x,y,1)),orig.Pixel(x,y,2));
	if (maxc==0.0)
	{
...
void FloatBitMap_t::CompressTo8Bits(float overbright)
{
	...
	for(int y=0;y<Height;y++)
		for(int x=0;x<Width;x++)
		{
			...
			float maxc=std::max(std::max(Pixel(x,y,0),Pixel(x,y,1)),Pixel(x,y,2));
			if (maxc==0)
			{
				for(int c=0;c<4;c++)
