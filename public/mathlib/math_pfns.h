...
FORCEINLINE float VECTORCALL FastSqrt( float x ) { return sqrtf( x ); }
FORCEINLINE float VECTORCALL FastRSqrt( float x ) { return 1.0f / sqrtf( x ); }
FORCEINLINE float VECTORCALL FastRSqrtFast( float x ) { return 1.0f / sqrtf( x ); }
void FastSinCos( float x, float* s, float* c );
float FastCos( float x );
inline float FastRecip( float x ) { return 1.0f / x; }
inline float FastSqrtEst( float x ) { return sqrtf( x ); }

#elif !(defined( PLATFORM_PPC ) || defined(SPU))
