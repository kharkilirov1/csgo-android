...
//	kFlags [in] flags for the zone (same as those passed to tmEnter
//	kpFormat [in] name of the zone (same as those passed to tmEnter. This may contain printf-style format specifiers.
// <time.h> defines a BSD-compatibility TM_ZONE macro that expands to tm_zone.
// Pull it in first (it is include-guarded) and drop that define so our zone
// macro wins regardless of header ordering in the translation unit.
#include <time.h>
#ifdef TM_ZONE
	#undef TM_ZONE
#endif
...
// Telemetry zone flags are only declared by the RAD headers; provide the
// no-op values used by call sites when telemetry is compiled out.
#ifndef TMZF_NONE
	#define TMZF_NONE 0
	#define TMZF_STALL 1
	#define TMZF_IDLE 2
